  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200 mt-6 sm:mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {error && <p className="text-red-600 text-sm font-medium bg-red-50 border border-red-100 p-3 rounded-xl">{error}</p>}

        <button 
          type="submit" disabled={isProcessing}
          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-medium transition-all shadow-md shadow-indigo-200 disabled:opacity-50 mt-4"
        >
          {isProcessing ? 'Memproses...' : (isLogin ? 'Masuk ke Dashboard' : 'Daftar Akun Baru')}
        </button>
      </form>

      <div className="mt-6 text-center text-sm text-slate-600">
        {isLogin ? "Belum punya akun?" : "Sudah punya akun?"}
        <button onClick={() => {setIsLogin(!isLogin); setError('');}} className="text-indigo-600 font-bold ml-1 hover:text-indigo-700 hover:underline">
